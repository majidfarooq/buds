...
<head>
  ...
  <link href="{{asset('public/assets/backend/vendor/fontawesome-free/css/all.min.css')}}" rel="stylesheet"
      type="text/css">
  <link
  ...
  <link href="{{asset('public/assets/backend/css/admin_style.css')}}" rel="stylesheet">
            ...
            <div class="col-lg-12">
              <div class="p-5" style="text-align: center;">
                ...
                @if ($message = \Illuminate\Support\Facades\Session::get('login_error'))
                  <div class="alert alert-danger" style="margin: 15px;"><button type="button" class="close" data-dismiss="alert">×</button><strong>{{ $message }}</strong></div>
                @elseif ($message = \Illuminate\Support\Facades\Session::get('log_out'))
                  <div class="alert alert-success" style="margin: 15px;"><button type="button" class="close" data-dismiss="alert">×</button><strong>{{ $message }}</strong></div>
                @endif

                <form class="user login_form" action="{{route('admin.auth')}}" method="post">
                  @csrf
                  @method('POST')
                  <div class="form-group">
                    <input type="email" value="{{ old('email') }}" placeholder="Enter Email Address..." class="form-control form-control-user @error('email')  is-invalid @enderror" aria-describedby="email" autocomplete="email" id="email" name="email"  required autofocus>
                    @error('email')
                  ...
                  <div class="form-group">
                    <input autocomplete="current-password"
                         class="form-control lock form-control-user @error('password') is-invalid @enderror"
